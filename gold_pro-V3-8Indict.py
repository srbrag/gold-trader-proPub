...
if st.button('🎯 GENERATE ALPHA TRADE ORDER'):
    ...
    buy_score = 0
    if indicators["RSI"] < 35: buy_score += 1
    if indicators["MFI"] < 30: buy_score += 1
    if indicators["STOCH_K"] < 20: buy_score += 1
    if indicators["STOCK_RSI_K"] < 20: buy_score += 1
    if indicators["CRSI"] < 20: buy_score += 1
    if price < last[get_col(df, 'BBL')]: buy_score += 1
    if indicators["ADX"] > 25: buy_score += 1 # Confirming Trend Strength
    ...
    st.info(f"Confluence Score: {buy_score}/10 Indicators Aligning")
